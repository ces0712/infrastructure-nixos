...
DEPLOY_USER := env_var_or_default("DEPLOY_USER", "root")
...
help:
  ...
  @echo "  DEPLOY_USER=<user>    -> default: root"
...
bootstrap:
  ...
  @echo "If SSD boot stalls in initrd, keep ethernet connected and try SSHing to root@{{PI_HOST}} with the same admin key."

boot-source:
  PI_HOST={{PI_HOST}} BOOT_USER={{DEPLOY_USER}} IDENTITY_FILE={{IDENTITY_FILE}} ./scripts/boot-source.sh

restore:
  PI_HOST={{PI_HOST}} IDENTITY_FILE={{IDENTITY_FILE}} ./scripts/restore.sh
